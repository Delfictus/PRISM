...
impl GpuQuboSolver {
    ...
    pub fn new(device: Arc<CudaDevice>) -> Result<Self> {
        ...
        let ptx = std::fs::read_to_string(&ptx_path)
            .map_err(|e| PRCTError::GpuError(format!("Failed to load PTX from {}: {}", ptx_path, e)))?;
        ...
        let flip_kernel = device.get_func("quantum_evolution", "qubo_flip_batch_kernel")
            .ok_or_else(|| PRCTError::GpuError("Failed to get qubo_flip_batch_kernel".to_string()))?;

        let metropolis_kernel = device.get_func("quantum_evolution", "qubo_metropolis_kernel")
            .ok_or_else(|| PRCTError::GpuError("Failed to get qubo_metropolis_kernel".to_string()))?;
    ...
    pub fn solve(
        ...
        if initial_state.len() != num_vars {
            return Err(PRCTError::InvalidInput(
                format!("Initial state size {} != num_vars {}", initial_state.len(), num_vars)
            ));
        }
        ...
        println!("[GPU-QUBO]   Temperature: {:.3} → {:.3}", config.t_initial, config.t_final);
        ...
        let d_row_ptr = self.device.htod_copy(csr.row_ptr.clone())
            .map_err(|e| PRCTError::GpuError(format!("Failed to upload row_ptr: {}", e)))?;
        let d_col_idx = self.device.htod_copy(csr.col_idx.clone())
            .map_err(|e| PRCTError::GpuError(format!("Failed to upload col_idx: {}", e)))?;
        let d_values = self.device.htod_copy(csr.values.clone())
        ...
        let mut d_state_current = self.device.htod_copy(state_u8.clone())
            .map_err(|e| PRCTError::GpuError(format!("Failed to upload state: {}", e)))?;
        let mut d_state_best = self.device.htod_copy(state_u8)
            .map_err(|e| PRCTError::GpuError(format!("Failed to upload best state: {}", e)))?;

        // Allocate working buffers
        let d_delta_energy = self.device.alloc_zeros::<f64>(config.batch_size)
            .map_err(|e| PRCTError::GpuError(format!("Failed to allocate delta_energy: {}", e)))?;
        let d_flip_candidates = self.device.alloc_zeros::<i32>(config.batch_size)
            .map_err(|e| PRCTError::GpuError(format!("Failed to allocate flip_candidates: {}", e)))?;
        let mut d_best_energy = self.device.htod_copy(vec![f64::INFINITY])
            .map_err(|e| PRCTError::GpuError(format!("Failed to allocate best_energy: {}", e)))?;

        // Initialize RNG states
        let num_threads = 256;
        let num_blocks = (config.batch_size + num_threads - 1) / num_threads;
        let rng_state_size = config.batch_size * 64; // curandStatePhilox4_32_10_t is ~64 bytes
        let d_rng_states = self.device.alloc_zeros::<u8>(rng_state_size)
        ...
        for iter in 0..config.iterations {
            ...
            unsafe {
                ...
                ).map_err(|e| PRCTError::GpuError(format!("Flip kernel launch failed at iter {}: {}", iter, e)))?;
            }
            ...
            unsafe {
                ...
                ).map_err(|e| PRCTError::GpuError(format!("Metropolis kernel launch failed at iter {}: {}", iter, e)))?;
            }

            // Progress logging
            if (iter + 1) % 1000 == 0 {
                self.device.synchronize()
                    .map_err(|e| PRCTError::GpuError(format!("Sync failed at iter {}: {}", iter, e)))?;

                let elapsed = start.elapsed().as_secs_f64();
                let iter_per_sec = (iter + 1) as f64 / elapsed;
                println!("[GPU-QUBO]   Iter {}/{} | T={:.4} | {:.1} iter/s",
                    iter + 1, config.iterations, temperature, iter_per_sec);
            }
        }
        ...
        let best_state_u8 = self.device.dtoh_sync_copy(&d_state_best)
            .map_err(|e| PRCTError::GpuError(format!("Failed to download result: {}", e)))?;

        let best_energy_vec = self.device.dtoh_sync_copy(&d_best_energy)
...
pub fn qubo_solution_to_coloring(
    ...
    if qubo_solution.len() != num_vertices * num_colors {
        return Err(PRCTError::InvalidInput(
            format!("QUBO solution size {} != n*k {}", qubo_solution.len(), num_vertices * num_colors)
        ));
    }
    ...
    if num_conflicts > 0 {
        println!("[GPU-QUBO][WARN] {} vertices with constraint violations", num_conflicts);
    }
