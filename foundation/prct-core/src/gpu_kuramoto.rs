...
impl GpuKuramotoSolver {
    ...
    pub fn new(device: Arc<CudaDevice>) -> Result<Self> {
        ...
        device.load_ptx(ptx, "kuramoto", &["kuramoto_step_kernel", "compute_order_parameter_kernel"])
            .map_err(|e| PRCTError::GpuError(format!("PTX load failed: {:?}", e)))?;

        let kuramoto_step_fn = Arc::new(
            device.get_func("kuramoto", "kuramoto_step_kernel")
                .ok_or_else(|| PRCTError::GpuError("Failed to get kuramoto_step_kernel".to_string()))?
        );

        let order_parameter_fn = Arc::new(
            device.get_func("kuramoto", "compute_order_parameter_kernel")
                .ok_or_else(|| PRCTError::GpuError("Failed to get compute_order_parameter_kernel".to_string()))?
        );
    ...
    pub fn evolve(
        ...
        // Upload to GPU
        let mut d_phases_in = self.device.htod_copy(phases_f32.clone())
            .map_err(|e| PRCTError::GpuError(format!("Host to device copy failed: {:?}", e)))?;

        let mut d_phases_out = self.device.alloc_zeros::<f32>(n)
            .map_err(|e| PRCTError::GpuError(format!("Device allocation failed: {:?}", e)))?;

        let d_frequencies = self.device.htod_copy(frequencies_f32)
        ...
        for _ in 0..num_steps {
            ...
            unsafe {
                (*self.kuramoto_step_fn).clone().launch(cfg, params)
        ...
        // Download result
        let result_f32 = self.device.dtoh_sync_copy(&d_phases_in)
    ...
    pub fn compute_order_parameter(&self, phases: &[f64]) -> Result<f64> {
        ...
        unsafe {
            (*self.order_parameter_fn).clone().launch(cfg, params)
                .map_err(|e| PRCTError::GpuError(format!("Order parameter kernel launch failed: {:?}", e)))?;
        }

        // Download results
        let sum_real = self.device.dtoh_sync_copy(&d_sum_real)
            .map_err(|e| PRCTError::GpuError(format!("Sum real download failed: {:?}", e)))?;

        let sum_imag = self.device.dtoh_sync_copy(&d_sum_imag)
    ...
    fn test_gpu_kuramoto_evolution() {
        if let Ok(device) = CudaDevice::new(0) {
            if let Ok(solver) = GpuKuramotoSolver::new(device) {
                ...
                // Evolve
                solver.evolve(&mut phases, &frequencies, 1.0, 0.01, 100).unwrap();
