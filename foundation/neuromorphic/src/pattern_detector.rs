...
impl PatternDetector {
    ...
    fn update_history(&self, pattern: SpikePattern) -> Result<()> {
        let mut history = self.history.lock().expect("mutex poisoned: pattern_detector history");
    ...
    fn detect_synchrony(&self) -> Option<DetectedPattern> {
        ...
        let threshold = *self.adaptive_threshold.lock().expect("mutex poisoned: adaptive_threshold");
    ...
    fn detect_traveling_wave(&self) -> Option<DetectedPattern> {
        let history = self.history.lock().expect("mutex poisoned: pattern_detector history");
        if history.len() < 3 {
            return None;
        }
    ...
    fn update_statistics(&self, patterns: &[DetectedPattern], processing_time: std::time::Duration) -> Result<()> {
        let mut stats = self.pattern_statistics.lock().expect("mutex poisoned: pattern_statistics");
        ...
        if self.config.adaptive_threshold {
            let mut threshold = self.adaptive_threshold.lock().expect("mutex poisoned: adaptive_threshold");
    ...
    pub fn get_statistics(&self) -> PatternStatistics {
        self.pattern_statistics.lock().expect("mutex poisoned: pattern_statistics").clone()
    }

    /// Get current adaptive threshold
    pub fn get_threshold(&self) -> f64 {
        *self.adaptive_threshold.lock().expect("mutex poisoned: adaptive_threshold")
    }

    /// Set detection threshold manually
    pub fn set_threshold(&self, threshold: f64) {
        let mut adaptive_threshold = self.adaptive_threshold.lock().expect("mutex poisoned: adaptive_threshold");
        *adaptive_threshold = threshold.clamp(0.1, 1.0);
    }

    /// Reset pattern detector state
    pub fn reset(&self) {
        self.history.lock().expect("mutex poisoned: pattern_detector history").clear();
        self.pattern_id_counter.store(0, Ordering::Relaxed);
        self.failure_count.store(0, Ordering::Relaxed);
        self.success_count.store(0, Ordering::Relaxed);
        *self.pattern_statistics.lock().expect("mutex poisoned: pattern_statistics") = PatternStatistics::default();
    }
}
