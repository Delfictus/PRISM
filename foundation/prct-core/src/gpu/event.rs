    ...
    pub fn record(&self, name: &str, stream: &CudaStream) -> Result<()> {
        let mut events = self.events.lock()
            .map_err(|e| PRCTError::GpuError(
                format!("Failed to lock event registry: {}", e)
            ))?;

        let event = events.entry(name.to_string())
            .or_insert_with(|| {
                CudaEvent::new(&self.device, name.to_string())
...
mod tests {
    ...
    fn test_event_registry() {
        ...
        registry.record("test_event", &stream).expect("Failed to record");
        assert!(registry.has_event("test_event"));

        registry.wait("test_event", &stream).expect("Failed to wait");
    }
}
