...
pub fn select_diverse_training_set(
    ...
    let min_chromatic = history.iter().map(|s| s.chromatic_number).min().unwrap_or(1);
    let max_chromatic = history.iter().map(|s| s.chromatic_number).max().unwrap_or(min_chromatic + 1);
...
mod tests {
    ...
    fn test_select_training_set_small_history() {
        let history = vec![
            create_test_solution(100, 50),
            create_test_solution(95, 30),
        ];
        let result = select_training_set(&history, 10, true, 42);
