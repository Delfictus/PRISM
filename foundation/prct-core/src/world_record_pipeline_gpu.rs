...
#[cfg(feature = "cuda")]
use neuromorphic_engine::types::{SpikePattern, Spike, PatternMetadata};
#[cfg(feature = "cuda")]
...
impl GpuReservoirConflictPredictor {
    ...
    pub fn predict_gpu(
        ...
        let mut gpu_reservoir = GpuReservoirComputer::new_shared(
            reservoir_config,
            cuda_device,
        ).map_err(|e| PRCTError::NeuromorphicFailed(format!("GPU reservoir init failed: {}", e)))?;

        println!("[GPU-RESERVOIR] Training on {} historical colorings...", coloring_history.len());
        ...
        for solution in coloring_history {
            ...
            let spikes = solution.colors.iter().enumerate()
            ...
            let pattern = SpikePattern {
                ...
                metadata: PatternMetadata {
                    ...
                    pattern_type: Some(if solution.conflicts > 0 { "conflict".to_string() } else { "valid".to_string() }),
            ...
            println!("[RESERVOIR][GPU] Processing pattern with {} spikes on GPU", pattern.spikes.len());
        ...
        println!("[GPU-RESERVOIR]   GPU time: {:.2}ms", stats.total_processing_time_us / 1000.0);
        println!("[GPU-RESERVOIR]   Speedup: {:.1}x vs CPU", stats.speedup_vs_cpu);
        println!("[GPU-RESERVOIR]   Difficulty zones identified: {}", difficulty_zones.len());
