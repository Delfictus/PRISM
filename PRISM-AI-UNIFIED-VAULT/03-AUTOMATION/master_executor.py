...
LATTICE_REPORT = ARTIFACT_DIR / "mec" / "M3" / "lattice_report.json"
...
def compute_lattice_hash(state: Dict[str, object], gradient: Dict[str, float], entropy: float) -> str:
    hasher = hashlib.sha256()
    hasher.update(_float_bytes(float(state["free_energy"])))
    hasher.update(_float_bytes(float(state["exploration"])))
    hasher.update(_float_bytes(float(state["exploitation"])))
    hasher.update(_float_bytes(float(state["divergence"])))
    hasher.update(_float_bytes(float(state["confidence"])))
    hasher.update(_float_bytes(float(state["temperature"])))
    hasher.update(state["timestamp"].encode("utf-8"))
    hasher.update(_float_bytes(float(entropy)))
    hasher.update(_float_bytes(float(gradient["free_energy"])))
    hasher.update(_float_bytes(float(gradient["exploration"])))
    hasher.update(_float_bytes(float(gradient["exploitation"])))
    return hasher.hexdigest()


def build_sample_lattice_snapshot() -> Dict[str, object]:
    ...
    entropy = state["exploration"]
    hash_value = compute_lattice_hash(state, gradient, entropy)
    snapshot = {
        "state": state,
        "gradient": gradient,
        "stability": round(1.0 - state["divergence"], 3),
        "entropy": entropy,
        "hash": hash_value,
    }
    history = []
    for idx in range(1, 4):
        past = current_dt - timedelta(hours=idx)
        history.append(
            {
                "free_energy": round(0.826 - float(idx) * 0.003, 3),
                "exploration": round(0.55 - float(idx) * 0.01, 3),
                "exploitation": round(0.45 + float(idx) * 0.01, 3),
                "divergence": round(0.19 + float(idx) * 0.002, 3),
                "confidence": round(0.71 - float(idx) * 0.015, 3),
                "temperature": 0.94,
                "mode": "strict",
    ...
    return {
        ...
        "snapshot": snapshot,
        "history": history,
        "notes": "Sample lattice report generated by master executor (Phase M3 placeholder).",
    }


def ensure_lattice_report(use_sample_metrics: bool) -> None:
    if LATTICE_REPORT.exists():
        return
    if not use_sample_metrics:
        print("⚠️ Lattice report missing; skipping creation (sample metrics disabled).")
        return
    report = build_sample_lattice_snapshot()
    LATTICE_REPORT.parent.mkdir(parents=True, exist_ok=True)
    LATTICE_REPORT.write_text(json.dumps(report, indent=2), encoding="utf-8")
    print(f"🧭 Lattice report generated at {LATTICE_REPORT}")
...
def load_lattice_metadata(use_sample_metrics: bool) -> Dict[str, object]:
    lattice_hash = None
    mode = None
    stability = None
    entropy = None
    ...
    if LATTICE_REPORT.exists():
        try:
            report = json.loads(LATTICE_REPORT.read_text(encoding="utf-8"))
            snapshot = report.get("snapshot", {})
            lattice_hash = snapshot.get("hash")
            mode = snapshot.get("state", {}).get("mode")
            stability = snapshot.get("stability")
            entropy = snapshot.get("entropy")
        except json.JSONDecodeError as exc:
            print(f"⚠️ Failed to parse lattice report: {exc}")

    if lattice_hash is None:
        lattice_hash = "0" * 64
    if mode is None:
        mode = "strict" if use_sample_metrics else "unknown"
    ...
    if stability is not None:
        ...
        payload["stability"] = 0.82
    if entropy is not None:
        payload["entropy"] = entropy
    elif use_sample_metrics:
        payload["entropy"] = 0.58
    return payload
...
def create_determinism_manifest(use_sample_metrics: bool) -> Dict[str, object]:
    ...
    manifest = {
        ...
        "lattice": load_lattice_metadata(use_sample_metrics),
    }
    return manifest
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ensure_lattice_report(args.use_sample_metrics)
    ...
    summary.artifacts = {
        ...
        "lattice_report": str(LATTICE_REPORT),
