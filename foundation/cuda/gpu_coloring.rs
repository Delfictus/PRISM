...
impl GpuColoringEngine {
    ...
    pub fn new() -> Result<Self> {
        ...
        let sparse_kernel = Arc::new(module.load_function("_Z28sparse_parallel_coloring_csrPKiS0_PKfPiS3_Pfiiify")
            .map_err(|e| anyhow!("Failed to load sparse kernel: {:?}", e))?);

        let dense_kernel = Arc::new(module.load_function("_Z30dense_parallel_coloring_tensorPKfS0_PiS1_Pfiiify")
    ...
    fn color_sparse(
        ...
        let mut workspace_gpu: CudaSlice<f32> = stream.alloc_zeros(workspace_size)?;
        ...
        launch.arg(&mut workspace_gpu);  // Pass workspace for dynamic arrays
        launch.arg(&n_i32);
