    ...
    pub fn description(&self) -> String {
        match self {
            ...
            AdpControl::TogglePhase { phase, enabled } => {
                format!("{} phase {}", if *enabled { "Enable" } else { "Disable" }, phase)
            }
            AdpControl::AdjustAdpAlpha { new_alpha } => {
                format!("Set ADP alpha to {:.3}", new_alpha)
            }
            AdpControl::ResetToBaseline => {
                "Reset to baseline config".to_string()
            }
        }
    }
}
...
mod tests {
    ...
    fn test_event_serialization() {
        ...
        let _deserialized: TelemetryEvent = serde_json::from_str(&json).expect("Failed to deserialize");
    }
