...
impl MultiGpuDevicePool {
    ...
    pub fn new(device_ids: &[usize], enable_peer_access: bool) -> Result<Self> {
        ...
        println!("[MULTI-GPU][INIT] Initializing device pool with {} GPUs", device_ids.len());
        ...
        for &device_id in device_ids {
            ...
            println!("[MULTI-GPU][INIT] GPU {} initialized successfully", device_id);
        ...
        if enable_peer_access {
            println!("[MULTI-GPU][INIT] Enabling peer-to-peer access between {} GPUs", devices.len());
            Self::enable_peer_access(&devices)?;
        } else {
            println!("[MULTI-GPU][INIT] Peer access disabled, will use CPU staging for cross-GPU transfers");
        }

        println!("[MULTI-GPU][INIT] ✅ Device pool ready with {} GPUs", devices.len());
