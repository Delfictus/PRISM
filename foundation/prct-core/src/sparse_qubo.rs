...
impl SparseQUBO {
    ...
    pub fn from_graph_coloring_with_color_penalty(
        ...
        if color_penalty_weight > 0.0 {
            println!("[SPARSE-QUBO]   Color penalty: {:.3} (prefers lower colors)", color_penalty_weight);
        }
        ...
        println!("[SPARSE-QUBO]   One-color constraint entries: {}", entries.len());
        ...
        println!("[SPARSE-QUBO]   Conflict constraint entries: {}", edge_entries);
        ...
        let memory_mb = (entries.len() * std::mem::size_of::<(usize, usize, f64)>()) as f64 / (1024.0 * 1024.0);
...
impl ChromaticBounds {
    ...
    fn find_max_clique_cpu(adj_matrix: &Array2<bool>) -> usize {
        ...
        for &start in vertices.iter().take(n.min(100)) {
            ...
            let candidate_degrees: Vec<(usize, usize)> = candidates.iter()
                .map(|&v| {
                    let deg = candidates.iter().filter(|&&u| u != v && adj_matrix[[v, u]]).count();
                    (v, deg)
                })
                .collect();
