...
use crate::errors::*;
use crate::telemetry::{RunMetric, PhaseName};
use crate::hypertune::action::{TelemetryEvent, AdpControl};
...
impl HypertuneController {
    ...
    fn process_metric(&mut self, metric: RunMetric) -> Result<()> {
        ...
        for event in events {
            if let Some(action) = self.choose_action(&event) {
                eprintln!("[HYPERTUNE] Event: {:?} -> Action: {}", event, action.description());
    ...
    fn detect_events(&self) -> Vec<TelemetryEvent> {
        ...
        for phase in [
            ...
            let phase_metrics: Vec<_> = self.metric_buffer
                .iter()
        ...
        match event {
            TelemetryEvent::PhaseStalled { phase, .. } => {
                match phase {
                    ...
                    PhaseName::Quantum => {
                        Some(AdpControl::IncreaseQuantumIterations { additional_iters: 100 })
                    }
                    PhaseName::Memetic => {
                        Some(AdpControl::AdjustMemeticPopulation { delta: 10 })
                    }
                    _ => None,
                }
            }
            ...
            TelemetryEvent::HighConflicts { phase, .. } => {
                match phase {
                    PhaseName::Thermodynamic => {
                        Some(AdpControl::AdjustThermoTemps { delta_percent: -10 })
                    }
