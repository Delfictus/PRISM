...
impl GpuQuantumSolver {
    ...
    pub fn new(device: Arc<CudaDevice>) -> Result<Self> {
        ...
        device.load_ptx(
            ptx,
            "quantum",
            &["complex_matvec", "complex_axpy", "complex_norm_squared_kernel", "complex_normalize"],
        ).map_err(|e| PRCTError::GpuError(format!("PTX load failed: {:?}", e)))?;

        let matvec_fn = Arc::new(
            device.get_func("quantum", "complex_matvec")
                .ok_or_else(|| PRCTError::GpuError("Failed to get complex_matvec".to_string()))?
        );

        let axpy_fn = Arc::new(
            device.get_func("quantum", "complex_axpy")
                .ok_or_else(|| PRCTError::GpuError("Failed to get complex_axpy".to_string()))?
        );

        let norm_sq_fn = Arc::new(
            device.get_func("quantum", "complex_norm_squared_kernel")
                .ok_or_else(|| PRCTError::GpuError("Failed to get complex_norm_squared_kernel".to_string()))?
        );

        let normalize_fn = Arc::new(
            device.get_func("quantum", "complex_normalize")
                .ok_or_else(|| PRCTError::GpuError("Failed to get complex_normalize".to_string()))?
        );
    ...
    pub fn evolve_state_gpu(
        ...
        for _ in 0..num_steps {
            ...
            // Normalize state
            let mut d_norm_sq = self.device.alloc_zeros::<f32>(1)
                .map_err(|e| PRCTError::GpuError(format!("Norm allocation failed: {:?}", e)))?;

            unsafe {
                (*self.norm_sq_fn).clone().launch(
                    cfg,
                    (
                        &d_state_re,
                        &d_state_im,
                ...
                ).map_err(|e| PRCTError::GpuError(format!("Norm kernel launch failed: {:?}", e)))?;
            }

            let norm_sq_vec = self.device.dtoh_sync_copy(&d_norm_sq)
                .map_err(|e| PRCTError::GpuError(format!("Norm download failed: {:?}", e)))?;

            let norm = norm_sq_vec[0].sqrt();

            unsafe {
                (*self.normalize_fn).clone().launch(
                    cfg,
                    (
                        &mut d_state_re,
                        &mut d_state_im,
                ...
                ).map_err(|e| PRCTError::GpuError(format!("Normalize launch failed: {:?}", e)))?;
            }
        }

        // Download final state
        let final_re = self.device.dtoh_sync_copy(&d_state_re)
            .map_err(|e| PRCTError::GpuError(format!("State_re download failed: {:?}", e)))?;

        let final_im = self.device.dtoh_sync_copy(&d_state_im)
