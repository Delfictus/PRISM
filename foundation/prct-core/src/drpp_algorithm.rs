...
impl DrppPrctAlgorithm {
    ...
    fn apply_drpp_enhancement(
        ...
        let n = neuro_series.len().min(quantum_series.len()).min(phase_field.phases.len());
        if n < 2 {
            return Err(PRCTError::DrppFailed("Insufficient data for DRPP enhancement".into()));
        }
        ...
        for step in 0..self.config.drpp_evolution_steps {
            ...
            for i in 0..n {
                ...
                let diffusion = 0.001 * (2.0 * (step as f64 / self.config.drpp_evolution_steps as f64) - 1.0);
        ...
        phase_field.order_parameter = evolved_phases.iter()
