...
impl PrismPipeline {
    ...
    pub fn new(config: PrismConfig) -> Result<Self> {
        ...
        let fusion_kernel = Arc::new(module.load_function("_Z23fuse_coherence_matricesPKfS0_S0_S0_Pfiffff")
            .map_err(|e| anyhow!("Failed to load fuse_coherence_matrices: {:?}", e))?);

        let init_kernel = Arc::new(module.load_function("_Z22init_uniform_coherencePfif")
