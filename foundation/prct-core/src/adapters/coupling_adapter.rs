...
impl CouplingAdapter {
    ...
    pub fn new(coupling_strength: f64) -> Result<Self> {
        if coupling_strength < 0.0 {
            return Err(PRCTError::CouplingFailed("Coupling strength must be non-negative".to_string()));
        }
...
impl PhysicsCouplingPort for CouplingAdapter {
    ...
    fn update_kuramoto_sync(
        ...
        #[cfg(feature = "cuda")]
        {
            if let Some(ref gpu_solver) = self.gpu_solver {
                ...
                gpu_solver.evolve(&mut phases, &natural_frequencies, self.coupling_strength, step_dt, num_steps)?;
            } else {
                // Fall back to CPU
                for _ in 0..num_steps {
                    self.service.kuramoto_step(&mut phases, &natural_frequencies, step_dt)?;
                }
            }
        }
    ...
    fn calculate_transfer_entropy(
        ...
        if source.is_empty() || target.is_empty() {
            return Err(PRCTError::CouplingFailed("Source and target must be non-empty".to_string()));
        }

        if source.len() != target.len() {
            return Err(PRCTError::CouplingFailed("Source and target must have same length".to_string()));
        }
    ...
    fn get_bidirectional_coupling(
        ...
        let coupling_quality = (
            neuro_to_quantum_entropy.entropy_bits.abs() +
...
mod tests {
    ...
    fn test_coupling_strength_calculation() {
        ...
        let coupling = adapter.compute_coupling(&neuro_state, &quantum_state)
    ...
    fn test_kuramoto_synchronization() {
        ...
        let kuramoto = adapter.update_kuramoto_sync(&neuro_phases, &quantum_phases, 0.1)
    ...
    fn test_transfer_entropy() {
        ...
        let te = adapter.calculate_transfer_entropy(&source, &target, 5.0)
    ...
    fn test_bidirectional_coupling() {
        ...
        let coupling = adapter.get_bidirectional_coupling(&neuro_state, &quantum_state)
