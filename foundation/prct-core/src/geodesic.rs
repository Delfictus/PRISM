...
pub fn compute_landmark_distances(
    ...
    if num_landmarks == 0 {
        return Err(PRCTError::ColoringFailed("num_landmarks must be > 0".to_string()));
    }

    let num_landmarks = num_landmarks.min(n);

    println!("[GEODESIC] Computing geodesic features: {} landmarks, metric={}", num_landmarks, metric);

    // Select landmarks (degree-based selection for diversity)
    let landmarks = select_landmarks(graph, num_landmarks)?;

    println!("[GEODESIC] Selected landmarks: {:?}", &landmarks[..5.min(landmarks.len())]);
    ...
    for v in 0..n {
        let max_dist = landmark_distances[v].iter()
            .copied()
...
fn compute_dijkstra_distances(graph: &Graph, source: usize) -> Result<Vec<f64>> {
    ...
    distances[source] = 0.0;
    heap.push(DijkstraState { vertex: source, distance: 0.0 });
    ...
    while let Some(DijkstraState { vertex: v, distance: dist }) = heap.pop() {
        ...
        for u in 0..n {
            if graph.adjacency[v * n + u] {
                ...
                if new_dist < distances[u] {
                    ...
                    heap.push(DijkstraState { vertex: u, distance: new_dist });
                }
            }
        }
    }
...
mod tests {
    ...
    fn test_geodesic_path() {
        ...
        let max_ecc = features.eccentricity.iter()
            .copied()
            .max_by(|a, b| a.partial_cmp(b).unwrap_or(Ordering::Equal))
            .unwrap_or(0.0);

        assert!(features.eccentricity[0] >= max_ecc * 0.9 || features.eccentricity[3] >= max_ecc * 0.9);
    }
}
