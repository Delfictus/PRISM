...
def generate_lattice_report() -> None:
    cmd = [
        "cargo",
        "run",
        "--quiet",
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "Failed to generate lattice report:\n" + result.stderr.strip()
        )
...
def load_lattice_metadata() -> Dict[str, object]:
    if not LATTICE_REPORT.exists():
        raise RuntimeError(f"Lattice report missing: {LATTICE_REPORT}")
    try:
        report = json.loads(LATTICE_REPORT.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise RuntimeError(f"Invalid lattice report JSON: {exc}") from exc

    snapshot = report.get("snapshot")
    if not isinstance(snapshot, dict):
        raise RuntimeError("Lattice report missing snapshot block")

    lattice_hash = snapshot.get("hash")
    mode = snapshot.get("state", {}).get("mode") if isinstance(snapshot.get("state"), dict) else None
    stability = snapshot.get("stability")
    entropy = snapshot.get("entropy")

    if not isinstance(lattice_hash, str):
        raise RuntimeError("Lattice report missing hash")
    if not isinstance(mode, str):
        raise RuntimeError("Lattice report missing reflexive mode")
    if stability is None or entropy is None:
        raise RuntimeError("Lattice report missing stability/entropy")
