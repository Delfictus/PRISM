...
impl CascadingPipeline {
    ...
    pub fn optimize(
        ...
        self.te_ordering = Some(hybrid_te_kuramoto_ordering(graph, initial_kuramoto, self.geodesic_features.as_ref(), 0.2)?);
        ...
        println!("[PHASE 1] ✅ TE-guided greedy: {} colors", te_solution.chromatic_number);

        self.best_solution = te_solution.clone();
        self.phase_history.push((1, self.best_solution.chromatic_number));
        ...
        if dsatur_solution.conflicts == 0 && dsatur_solution.chromatic_number < self.best_solution.chromatic_number {
            println!("[PHASE 2] ✅ DSATUR improved: {} → {} colors",
                     self.best_solution.chromatic_number, dsatur_solution.chromatic_number);
            self.best_solution = dsatur_solution;
            self.stagnation_count = 0;
        } else {
            println!("[PHASE 2] ⚠️  DSATUR: {} colors (no improvement)",
                     dsatur_solution.chromatic_number);
            self.stagnation_count += 1;
        }

        self.phase_history.push((2, self.best_solution.chromatic_number));
        ...
        self.kuramoto_state = Some(self.compute_kuramoto_from_coloring(graph, &self.best_solution)?);
        // Safe: just set above; expect clarifies invariant
        ...
        if kuramoto_solution.conflicts == 0 && kuramoto_solution.chromatic_number < self.best_solution.chromatic_number {
            println!("[PHASE 3] ✅ Kuramoto improved: {} → {} colors",
                     self.best_solution.chromatic_number, kuramoto_solution.chromatic_number);
            self.best_solution = kuramoto_solution.clone();
            self.stagnation_count = 0;
        } else {
            println!("[PHASE 3] Kuramoto: {} colors (no improvement)", kuramoto_solution.chromatic_number);
            self.stagnation_count += 1;
        }

        self.phase_history.push((3, self.best_solution.chromatic_number));
        ...
        if memetic_solution.conflicts == 0 && memetic_solution.chromatic_number < self.best_solution.chromatic_number {
            println!("[PHASE 4] ✅ Memetic improved: {} → {} colors",
                     self.best_solution.chromatic_number, memetic_solution.chromatic_number);
            self.best_solution = memetic_solution;
            self.stagnation_count = 0;
        } else {
            println!("[PHASE 4] Memetic: {} colors (no improvement)", memetic_solution.chromatic_number);
            self.stagnation_count += 1;
        }

        self.phase_history.push((4, self.best_solution.chromatic_number));
        ...
        let mut final_dsatur = DSaturSolver::new(self.best_solution.chromatic_number, final_dsatur_depth)
            .with_kuramoto_phases(kuramoto_state.phases.clone());

        let final_solution = final_dsatur.find_coloring(graph, Some(&self.best_solution), dsatur_target)?;

        if final_solution.conflicts == 0 && final_solution.chromatic_number < self.best_solution.chromatic_number {
            println!("[PHASE 5] ✅ Final DSATUR improved: {} → {} colors",
                     self.best_solution.chromatic_number, final_solution.chromatic_number);
            self.best_solution = final_solution;
        } else {
            println!("[PHASE 5] Final DSATUR: {} colors (no improvement)", final_solution.chromatic_number);
        }

        self.phase_history.push((5, self.best_solution.chromatic_number));

        // ===== LOOPBACK CHECK =====
        if self.stagnation_count >= 3 && self.phase_history.len() < self.max_iterations {
            println!("\n⚠️  Stagnation detected ({} phases without improvement)", self.stagnation_count);
        ...
        println!("╚═══════════════════════════════════════════════════════════╝");
        println!("[PIPELINE] Final result: {} colors", self.best_solution.chromatic_number);
