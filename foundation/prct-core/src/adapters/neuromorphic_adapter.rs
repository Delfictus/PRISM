...
use crate::ports::{NeuromorphicPort, NeuromorphicEncodingParams};
use crate::errors::{Result, PRCTError};
use shared_types::*;
use std::sync::{Arc, Mutex};

#[cfg(feature = "cuda")]
use neuromorphic_engine::{SpikeEncoder, EncodingParameters};
#[cfg(feature = "cuda")]
...
use neuromorphic_engine::{ReservoirComputer, SpikeEncoder, EncodingParameters};
#[cfg(not(feature = "cuda"))]
use neuromorphic_engine::reservoir::ReservoirConfig;
...
impl NeuromorphicAdapter {
    ...
    pub fn new(cuda_device: Arc<CudaDevice>) -> Result<Self> {
        ...
        let gpu_reservoir = GpuReservoirComputer::new_shared(reservoir_config, cuda_device)
            .map_err(|e| PRCTError::NeuromorphicFailed(format!("Failed to create GPU reservoir: {}", e)))?;
    ...
    fn graph_to_spike_pattern(&mut self, graph: &Graph, params: &NeuromorphicEncodingParams) -> Result<neuromorphic_engine::SpikePattern> {
        ...
        let input_data = neuromorphic_engine::types::InputData::new(
            "graph_topology".to_string(),
            values,
        );
        ...
        let mut encoder = SpikeEncoder::new(100, 100.0)
            .map_err(|e| PRCTError::NeuromorphicFailed(format!("Failed to create spike encoder: {}", e)))?;
...
impl NeuromorphicPort for NeuromorphicAdapter {
    fn encode_graph_as_spikes(
        ...
        let spikes: Vec<Spike> = neuro_spike_pattern.spikes
            .iter()
            .map(|s| Spike {
        ...
        let num_neurons = neuro_spike_pattern.spikes
            .iter()
            .map(|s| s.neuron_id)
            .max()
            .map(|max_id| max_id + 1)
    ...
    fn process_and_detect_patterns(&self, spikes: &SpikePattern) -> Result<NeuroState> {
        ...
        let neuro_spikes: Vec<neuromorphic_engine::types::Spike> = spikes.spikes
            .iter()
        ...
        let spike_density = num_spikes as f64 / (spikes.duration_ms * spikes.num_neurons as f64).max(1.0);
...
mod tests {
    ...
    fn test_neuromorphic_adapter_gpu() {
        ...
        let spikes = adapter.encode_graph_as_spikes(&graph, &params)
            .expect("encoding");
