...
impl DSaturSolver {
    ...
    pub fn find_coloring(
        ...
        if self.backtracking_search(&mut initial_state, &adjacency, &mut best_coloring, target_colors) {
            println!("[DSATUR] ✅ Found valid coloring with {} colors", self.best_chromatic);
        } else {
        ...
        let chromatic_number = best_coloring.iter().filter(|&&c| c != usize::MAX).map(|&c| c + 1).max().unwrap_or(0);
    ...
    fn backtracking_search(
        ...
        if self.nodes_explored % 10000 == 0 {
            println!("[DSATUR] Explored {} nodes, best: {} colors, depth: {}, backtracks: {}",
        ...
        if state.vertices.iter().all(|v| v.color != usize::MAX) {
            ...
            if chromatic < self.best_chromatic {
                ...
                println!("[DSATUR] 🎯 New best: {} colors (nodes: {}, backtracks: {})",
    ...
    fn select_next_vertex(&self, state: &SearchState) -> usize {
        ...
        for (i, vertex) in state.vertices.iter().enumerate() {
            ...
            let reservoir_score = if let Some(ref scores) = self.reservoir_conflict_scores {
                if i < scores.len() { scores[i] } else { 0.0 }
            } else {
                0.0
            };
            ...
            let efe_score = if let Some(ref efe) = self.active_inference_efe {
                if i < efe.len() { efe[i] } else { f64::MAX }
            } else {
                f64::MAX
            };
