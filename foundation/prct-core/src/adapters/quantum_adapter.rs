...
use crate::errors::{Result, PRCTError};
use shared_types::*;
use std::sync::Arc;
...
impl QuantumAdapter {
    ...
    fn build_hamiltonian_matrix(&self, coupling: &Array2<Complex64>, params: &EvolutionParams) -> Array2<Complex64> {
    ...
    fn compute_eigenvalues(&self, hamiltonian: &Array2<Complex64>) -> Vec<f64> {
        ...
        for i in 0..n {
            ...
            let off_diag_correction: f64 = (0..n)
                .filter(|&j| i != j)
                .map(|j| hamiltonian[[i, j]].norm_sqr() / (diag - hamiltonian[[j, j]].re).abs().max(1e-10))
                .sum();
    ...
    fn evolve_quantum_state(
        ...
        let phase_coherence = if norm_sqr > 1e-10 {
            let mean_phase = final_state_vec.iter()
                .map(|c| c.arg())
                .sum::<f64>() / n as f64;

            let coherence = final_state_vec.iter()
        ...
        let amplitudes: Vec<(f64, f64)> = final_state_vec.iter()
            .map(|c| (c.re, c.im))
            .collect();
    ...
    fn extract_phase_field(&self, state: &QuantumState) -> PhaseField {
        ...
        let order_parameter = phases.iter()
...
impl QuantumPort for QuantumAdapter {
    fn build_hamiltonian(&self, graph: &Graph, params: &EvolutionParams) -> Result<HamiltonianState> {
        ...
        if n == 0 {
            return Err(PRCTError::QuantumFailed("Cannot build Hamiltonian for empty graph".to_string()));
        }
        ...
        let matrix_elements: Vec<(f64, f64)> = hamiltonian.iter()
            .map(|c| (c.re, c.im))
            .collect();
    ...
    fn evolve_state(
        ...
        let hamiltonian_matrix = Array2::from_shape_vec(
            ...
            hamiltonian.matrix_elements
                ...
                .map(|(re, im)| Complex64::new(*re, *im))
                .collect()
        ).map_err(|e| PRCTError::QuantumFailed(format!("Failed to reconstruct Hamiltonian: {}", e)))?;

        // Evolve state
        let evolved_state = self.evolve_quantum_state(&hamiltonian_matrix, initial_state, evolution_time);
...
mod tests {
    ...
    fn test_quantum_evolution() {
        ...
        let hamiltonian = adapter.build_hamiltonian(&graph, &params).expect("hamiltonian");
        let ground_state = adapter.compute_ground_state(&hamiltonian).expect("ground state");

        let evolved = adapter.evolve_state(&hamiltonian, &ground_state, 0.1).expect("evolution");
