...
pub fn quantum_annealing_multi_gpu(
    ...
    if num_gpus == 0 {
        return Err(PRCTError::GpuError("No GPUs available for multi-GPU quantum".to_string()));
    }

    println!("[QUANTUM-MULTI-GPU] Distributing {} attempts across {} GPUs",
             total_attempts, num_gpus);
    println!("[QUANTUM-MULTI-GPU] {} attempts per GPU (approx)", total_attempts / num_gpus);
    println!("[QUANTUM-MULTI-GPU] Depth: {}, iterations per attempt: {}", depth, depth * 1000);
        ...
        match handle.join() {
            Ok(Ok((gpu_idx, state, energy))) => {
                ...
                if energy < global_best_energy {
                    ...
                    println!("[QUANTUM-MULTI-GPU] GPU {} found new global best: {:.6}",
                             gpu_idx, energy);
                }
            }
            Ok(Err(e)) => {
    ...
    println!("[QUANTUM-MULTI-GPU] ✅ Best solution from {} total attempts: energy={:.6}",
    ...
    for (gpu_idx, energy) in gpu_energies.iter().enumerate() {
        if *energy != f64::MAX {
            ...
            println!("[QUANTUM-MULTI-GPU]   GPU {}: {:.6} ({:+.2}%)",
